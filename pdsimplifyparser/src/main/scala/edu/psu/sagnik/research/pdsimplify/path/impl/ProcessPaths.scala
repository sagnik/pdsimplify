...
class ProcessPaths(page: PDPage) extends PDFGraphicsStreamEngine(page: PDPage) {
  ...
  def appendRectangle(p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D): Unit = {
    ...
    (currentPath, currentSubPath) match {
      case (Some(cp), Some(csp)) =>
        currentPath = Some( //a current path exists, add this whole subpath to the subpaths.
          cp.copy(
        ...
        currentSubPath = None
      case (None, Some(csp)) =>
        currentPath = Some( //this is the beginning of a new path
          PDPath(
            subPaths = List(csp),
            isClip = false,
            doPaint = true,
  ...
  @Override @throws[IOException] //TODO: Revisit, this might be important from the clipping perspective
